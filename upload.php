...
$checkGameSql = "SELECT COUNT(*) FROM igre WHERE LOWER(ime) = LOWER(:ime)";
$stmtCheck = $conn->prepare($checkGameSql);
$stmtCheck->bindParam(':ime', $ime, PDO::PARAM_STR);
$stmtCheck->execute();
$gameExists = $stmtCheck->fetchColumn();

if ($gameExists > 0) {
    // A game with the same name already exists, handle the error
    setcookie('prijava', 'Igra z istim imenom že obstaja.');
    setcookie('error', 1);
    header('Location: addgame.php');
    exit();
} else {
    // Continue with inserting the game into the database
    $insertGameSql = "INSERT INTO igre (ime, opis, uporabnik_id, zanr_id, cena) VALUES (:ime, :opis, :publisher, :zanr, :cena);";
